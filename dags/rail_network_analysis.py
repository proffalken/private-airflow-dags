...
# S3 bucket used to stage downloaded NROD files between the fetch and parse tasks.
# Storing files in Garage rather than worker-pod local /tmp means fetch and parse
# can run on different pods (CeleryExecutor) without losing the file.
...
def fetch_corpus(ti) -> str:
    ...
    with instrument_task_context({"nrod.feed": "CORPUS"}) as span:
        ...
        object_key = f"nrod/corpus_{ti.run_id}.json.gz"
        try:
            with otel_task_tracer.start_child_span(span_name="rail.stage.corpus") as stage_span:
                s3 = S3Hook(aws_conn_id="garage_s3")
                s3.load_file(filename=tmp.name, key=object_key, bucket_name=STAGING_BUCKET, replace=True)
                stage_span.set_attribute("s3.key", object_key)
                stage_span.set_attribute("s3.bucket", STAGING_BUCKET)
                logger.info("CORPUS staged to s3://%s/%s", STAGING_BUCKET, object_key)
        finally:
            os.unlink(tmp.name)
...
def fetch_schedule(ti) -> str:
    ...
    with instrument_task_context({"nrod.feed": "CIF_FULL"}) as span:
        ...
        object_key = f"nrod/cif_{ti.run_id}.cif.gz"
        try:
            with otel_task_tracer.start_child_span(span_name="rail.stage.schedule") as stage_span:
                s3 = S3Hook(aws_conn_id="garage_s3")
                s3.load_file(filename=tmp.name, key=object_key, bucket_name=STAGING_BUCKET, replace=True)
                stage_span.set_attribute("s3.key", object_key)
                stage_span.set_attribute("s3.bucket", STAGING_BUCKET)
                logger.info("CIF staged to s3://%s/%s", STAGING_BUCKET, object_key)
        finally:
            os.unlink(tmp.name)
...
@task
def parse_corpus(corpus_key: str) -> dict[str, Any]:
    """Download CORPUS from S3 staging, parse, and upsert into rail_locations.
    ...
    """
    logger.info("Parsing CORPUS from s3://%s/%s", STAGING_BUCKET, corpus_key)
    otel_task_tracer = otel_tracer.get_otel_tracer_for_task(Trace)

    s3 = S3Hook(aws_conn_id="garage_s3")
    with otel_task_tracer.start_child_span(span_name="rail.retrieve.corpus") as retrieve_span:
        corpus_path = s3.download_file(
            key=corpus_key,
            bucket_name=STAGING_BUCKET,
            preserve_file_name=True,
            use_autogenerated_subdir=False,
        )
        retrieve_span.set_attribute("s3.key", corpus_key)
        retrieve_span.set_attribute("s3.bucket", STAGING_BUCKET)
        logger.info("CORPUS retrieved from S3 to %s", corpus_path)
    ...
    with otel_task_tracer.start_child_span(span_name="rail.cleanup.corpus") as cleanup_span:
        s3.delete_objects(bucket=STAGING_BUCKET, keys=[corpus_key])
        cleanup_span.set_attribute("s3.key", corpus_key)
...
@task
def parse_schedule(schedule_key: str) -> dict[str, Any]:
    """Download CIF timetable from S3 staging, stream-parse, and insert into rail_schedules + rail_schedule_stops.
    ...
    logger.info("Parsing CIF schedule from s3://%s/%s (run_date=%s)", STAGING_BUCKET, schedule_key, run_date)
    otel_task_tracer = otel_tracer.get_otel_tracer_for_task(Trace)

    s3 = S3Hook(aws_conn_id="garage_s3")
    with otel_task_tracer.start_child_span(span_name="rail.retrieve.schedule") as retrieve_span:
        schedule_path = s3.download_file(
            key=schedule_key,
            bucket_name=STAGING_BUCKET,
            preserve_file_name=True,
            use_autogenerated_subdir=False,
        )
        retrieve_span.set_attribute("s3.key", schedule_key)
        retrieve_span.set_attribute("s3.bucket", STAGING_BUCKET)
        logger.info("CIF retrieved from S3 to %s", schedule_path)
    ...
    with otel_task_tracer.start_child_span(span_name="rail.cleanup.schedule") as cleanup_span:
        s3.delete_objects(bucket=STAGING_BUCKET, keys=[schedule_key])
        cleanup_span.set_attribute("s3.key", schedule_key)
...
def rail_network_analysis():
    ...
    corpus_key = fetch_corpus()
    schedule_key = fetch_schedule()

    # parse tasks each depend on exactly one fetch result, maintaining the
    # parallelism through the parse phase.
    corpus_result = parse_corpus(corpus_key)
    parse_result = parse_schedule(schedule_key)
