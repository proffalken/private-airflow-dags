...
def parse_corpus(corpus_key: str) -> dict[str, Any]:
    ...
    s3 = S3Hook(aws_conn_id="garage_s3")
    corpus_tmp = tempfile.NamedTemporaryFile(suffix=".json.gz", delete=False)
    corpus_tmp.close()
    s3.get_conn().download_file(STAGING_BUCKET, corpus_key, corpus_tmp.name)
...
def parse_schedule(schedule_key: str) -> dict[str, Any]:
    ...
    s3 = S3Hook(aws_conn_id="garage_s3")
    schedule_tmp = tempfile.NamedTemporaryFile(suffix=".cif.gz", delete=False)
    schedule_tmp.close()
    s3.get_conn().download_file(STAGING_BUCKET, schedule_key, schedule_tmp.name)
    schedule_path = schedule_tmp.name
